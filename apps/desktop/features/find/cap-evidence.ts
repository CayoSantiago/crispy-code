...
export function capSearchGroups(groups: SearchGroup[]): SearchGroup[] {
  ...
  for (const group of groups) {
    if (remainingFiles <= 0 || remainingMatches <= 0) break
...
function capFile(
  ...
  if (remainingMatches <= 0) return null
  ...
  for (const cluster of file.clusters) {
    ...
    if (clusterMatches === 0) continue
    if (matchCount + clusterMatches > remainingMatches) break
