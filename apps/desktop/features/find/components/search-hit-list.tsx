            ...
            <div className='grid grid-cols-1 gap-3'>
              {visibleFiles.map((file) => {
                ...
                const lineNum = Number(
                  decorations.find((item) => item.data?.lineNumber)?.data
                    ?.lineNumber,
                )
                const linkedLine =
                    ...
                    <div className='flex items-center max-w-full gap-1.5 p-2 pr-3 bg-muted h-10'>
                      ...
                      <div className='flex shrink-0 items-center gap-0.5 ml-auto'>
                        ...
                        <EditorLink
                          ...
                          href={`cursor://file/${encodeURIComponent(file.absolutePath)}:${firstMatchLineNumber(file)}`}
                        >
                          <CursorIcon />
                        </EditorLink>
                        <EditorLink
                          label='Open in Visual Studio Code'
                          href={`vscode://file/${encodeURIComponent(file.absolutePath)}:${firstMatchLineNumber(file)}`}
                        >
                          <VisualStudioCode />
                        </EditorLink>
                      </div>
...
function firstMatchLineNumber(file: SearchFile): number {
  for (const cluster of file.clusters) {
    for (const line of cluster.lines) {
      if (line.kind === 'match') return line.lineNumber
    }
  }
...
function formatSearchFileData(file: SearchFile) {
  ...
  let offset = 0
  const decorations = lines.flatMap((line, index) => {
    const start = offset
    offset += line.lineText.length + 1
      ...
      return [
        {
          lines: index + 1,
          className: 'text-muted-foreground',
        } satisfies HighlightDecoration,
      ]
    }
    return (
      line.matchRanges?.map(
        (range): HighlightDecoration => ({
