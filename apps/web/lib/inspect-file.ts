...
function detectMagic(bytes: Uint8Array): DetectedFormat | null {
  if (startsWith(bytes, RIFF) && bytesEqualAt(bytes, 8, WEBP)) {
    return 'webp'
  }
...
export function inspectFileBytes(bytes: Uint8Array): FileInspection {
  ...
  if (!bom && hasNul(bytes)) {
    return { kind: 'binary', format: null }
  }
  ...
  if (text !== null) return { kind: 'text', encoding, text, bom }
...
export function imageMimeType(format: DetectedFormat | null): ImageMime | null {
  if (!isImageFormat(format)) return null
  return IMAGE_MIME_TYPES[format as ImageFormat]
}
