...
export function AskThread({ threadId }: { threadId: string }) {
  ...
  const streamTurnId = useRef(runningTurnId)
  if (runningTurnId) {
    streamTurnId.current = runningTurnId
  }
  const live = useAskTurnStream(threadId, streamTurnId.current ?? runningTurnId)
    ...
    <>
      {data.turns.map((turn) => (
        <AskTurnItems
          ...
          turn={turn}
          thinking={
            turn.id === streamTurnId.current && turn.status !== 'FAILED'
              ? live.thinking
              : ''
          }
          liveAnswer={turn.id === streamTurnId.current ? live.answer : ''}
        />
      ))}
    </>
  )
