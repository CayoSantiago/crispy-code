...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        'no-scrollbar max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))] scroll-py-1 overflow-y-auto overscroll-contain p-0.75 data-empty:p-0',
        className,
      )}
      {...props}
    />
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "relative flex min-h-5 w-full cursor-default items-center gap-2 rounded-md px-1.5 py-0.75 text-xs/relaxed outline-hidden select-none data-highlighted:bg-accent! data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5 [&_svg:not([class*='stroke-'])]:stroke-[1.6]",
        className,
      )}
      {...props}
    >
